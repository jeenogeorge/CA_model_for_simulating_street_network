...
#ADD # BEFORE + for only drainage lines
strm_rd$newor <- replace_na(strm_rd$strahler,0)+replace_na(strm_rd$strahler_2,0)
#only ridge (DELETE ABOVE TWO LINES AND USE THE FOLLOWING LINE FOR 'ONLY RIDGE LINES')
...
for (i in 1:total){#begin running simulation
  ...
  model_TPP <- model_nhood*model_random*(strm_r_alt_n)*slope*(rd_1_co) #+ rd_acc*(model_nhood*model_random)
...
#PART 2
city_name <- "enter CITY_NAME"
#reading the stdy area extent which is same as singapore for all cities - do in QGIS
stdy_r <- raster(sprintf("xxx/%s/rasterised/stdy_r.grd",city_name)) 
plot(stdy_r)
res(stdy_r)
#reading road network of time 1
rd_netw_1_r <- raster(sprintf("xxx/%s/rasterised/rd_netw_1_r.grd",city_name)) 
plot(rd_netw_1_r)
res(rd_netw_1_r)

#reading road network of time 2
rd_netw_2_r <- raster(sprintf("xxx/%s/rasterised/rd_netw_2_r.grd",city_name)) 
plot(rd_netw_2_r)
res(rd_netw_2_r)


#reading the stream network
strm_rd <- readOGR(dsn = sprintf("xxx/%s",city_name), layer = "order_union")
plot(strm_rd)
...
#ADD # BEFORE + for only drainage lines
strm_rd$newor <- replace_na(strm_rd$strahler,0)+replace_na(strm_rd$strahler_2,0)
#only ridge delete the last two lines and use the following line for only ridge lines
#strm_rd$newor <- replace_na(strm_rd$strahler_2,0) #+ replace_na(strm_rd$strahler,0)
...
slope <-raster(sprintf("xxx/%s/rasterised/slope.grd",city_name)) 
slope
slope <- projectRaster(slope,stdy_r, method = 'ngb')
res(slope)
res(strm_r)
res(stdy_r)
res(strm_alt)
...
range(slope[], na.rm = T)
replace_na(slope,Inf)
slope <- reclassify(slope, c(0,5,5,5,20,.75,20,Inf,.1) )

fun <- function(x){x[x>=0] <- 1; return(x)}
rd_1 <- calc(rd_netw_1_r, fun)
plot(rd_1)
#write rd_1 and open in qgis to find the distance
writeRaster(rd_1, file = sprintf("xxx/%s/rasterised/rd_1.grd",city_name),overwrite = T)
res(rd_1)
#read road distance raster
rd_dist_1 <- raster(sprintf("xxx/%s/rasterised/rd_dist_1.grd",city_name)) 
plot(rd_dist_1)
res(rd_dist_1)
...
for (i in 1:total){#begin running simulation
  ...
  n <- focal(T1, w=w)
  nhood <- cover(n,T1)
  nhood[] <- (1+(nhood[]-min(nhood[]))*10/(max(nhood[]-min(nhood[]))))*0.25
  ...
  weibull <- 1+(-log(1-(x)))*exp(1/2)
  funselect <- function(x) {x[x!=0] <- NA; return(x)}
  vacant <- calc(T1,funselect)
  random <- rd_1
  values(random) <- weibull
  model_random <- mask(random, vacant)
  model_random <- cover(model_random, T1)
  model_random[] <- (1 + (model_random[]-min(model_random[]))*9/(max(model_random[])-min(model_random[])))*.025
  freq(model_random)
  #end random block
  #--------------------------------------
  #begin distance matrix for stream suitability
  ...
  #begin tansition potential calculation
  #exp 2
  model_TPP <- (strm_r)*((model_nhood*model_random))*slope*(rd_1_co) #+ rd_acc*(model_nhood*model_random)
  ...
  png(filename = paste(filen),width = 1200, height = 1200, bg="white") #Plot each land use map to be able to make an animation.
  #plot(T1)
  #plot(T1, breaks = breakpoints, col = colors, main = (start_year+i))
  #plot(T1, main = (start_year+i))
  writeRaster(T1, file = sprintf("xxx/%s/animate/%s.tif",city_name,(start_year+i)),overwrite = T)
  dev.off()
...
close(pb)
#png(filename = paste("rd", start_year, ".png", sep = ""), width = 1200, height = 1200,
#bg = "white")
rd_netw_1_r <- raster(sprintf("xxx/%s/rasterised/rd_netw_1_r.grd",city_name)) 
rd_1 <- replace_na(rd_1,0)
...
sim99 <- T1
sss <- stack(rd_2, sim99)
#name1 <- paste("initial land use", start_year, sep = "")
name <- paste("real road network", start_year, sep ="")
name1 <- paste("real road network", end_year, sep = "")
name2 <- paste("simulated road network", end_year, sep = "")
names(sss) <- c(name1, name2)
...
rd <- as.character(rd_2[])
sim
confusionMatrix(data = factor(sim), reference = factor(rd), positive = "1")
nlevels(factor(rd))

#confusion matrix for the added cells 
library(caret)
R_S <- jaccard(rd_2, sim99, x.min = 0.5, y.min = 0.5)
diff_sim <- (sim99-rd_1)
rd_1 <- raster(sprintf("xxx/%s/rasterised/real_rd_1.grd",city_name)) 
...
#after rasterising data just read file using the below codes
writeRaster(diff_rd, file = sprintf("xxx/%s/rasterised/10_B_2_diff_rd_10.rst",city_name),overwrite = T)
writeRaster(diff_sim, file = sprintf("xxx/%s/rasterised/10_B_2_diff_sim_10.rst",city_name),overwrite = T)
writeRaster(rd_1, file = sprintf("xxx/%s/rasterised/10_B_2_rd_1_10.rst",city_name),overwrite = T)
writeRaster(rd_2, file = sprintf("xxx/%s/rasterised/10_B_2_rd_2_10.rst",city_name),overwrite = T)
writeRaster(sim99, file = sprintf("xxx/%s/rasterised/10_B_2_sim99_10.rst",city_name),overwrite = T)
...
result_tab <- data.frame(name = city_name, 
                         ...
                         jacc_diff_real_sim = jaccard(diff_rd, diff_sim, x.min = 0.7, y.min = 0.7),
                         jacc_diff_neighofreal_sim = jaccard(diff_sim, nhood_2, x.min = 0.7, y.min = 0.7))
write.table(result_tab, file = 'xxx/result_tab_10', 
            append = T, sep = '\t', row.names = F, col.names = F)
...
#selecting only the higher order streams
#strm_rd$strahler <- replace_na(strm_rd$strahler,0)
strm_rd$strahler <- as.numeric(strm_rd$strahler)
strm_rd$strahler 
#strm_rd$strahler_2 <- replace_na(strm_rd$strahler_2,0)
strm_rd$strahler_2 <- as.numeric(strm_rd$strahler_2)
strm_rd$strahler_2
#adding all strahler metrics into a single column
#ADD # BEFORE + for only drainage lines
strm_rd$newor <- replace_na(strm_rd$strahler,0)+replace_na(strm_rd$strahler_2,0)
#only ridge delete the last two lines and add the following lines for only ridge lines
#strm_rd$newor <- replace_na(strm_rd$strahler_2,0) #+ replace_na(strm_rd$strahler,0)
...
ptm <- proc.time()
pb <- txtProgressBar(min = 0, max = total, style = 3)
writeRaster(rd_1, file = sprintf("E:/phd iit kgp/WORK/cellular_auto/%s/animate/%s.tif",city_name,(start_year)),overwrite = T)
#*****FOR THE TESTING TIME******
#total<-8
#list for stacking the rasters at each time step
s_t <- list()
for (i in 1:total){#begin running simulation
  ...
  #-----------------------------------------------
  T1_new <- nhood
  fun_T1_new_2 <- function(x){x[x<2] <- NA; return(x)}
  T1_new <- calc(T1_new, fun_T1_new_2)
  
  #exp_2
  model_TPP <- ((model_nhood*model_random))*(strm_alt)*slope*(rd_1_co) #+ rd_acc*(model_nhood*model_random)
...
#after rasterising data just read file using the below codes
writeRaster(diff_rd, file = sprintf("xxx/%s/rasterised/10_B_3_diff_rd_10.rst",city_name),overwrite = T)
writeRaster(diff_sim, file = sprintf("xxx/%s/rasterised/10_B_3_diff_sim_10.rst",city_name),overwrite = T)
writeRaster(rd_1, file = sprintf("xxx/%s/rasterised/10_B_3_rd_1_10.rst",city_name),overwrite = T)
writeRaster(rd_2, file = sprintf("xxx/%s/rasterised/10_B_3_rd_2_10.rst",city_name),overwrite = T)
writeRaster(sim99, file = sprintf("xxx/%s/rasterised/10_B_3_sim99_10.rst",city_name),overwrite = T)
...
#ADD # BEFORE + for only drainage lines
strm_rd$newor <- replace_na(strm_rd$strahler,0)+replace_na(strm_rd$strahler_2,0)
#only ridge delete the last two lines and add the following line for only ridge lines
#strm_rd$newor <- replace_na(strm_rd$strahler_2,0) #+ replace_na(strm_rd$strahler,0)
...
for (i in 1:total){#begin running simulation
  ...
  model_TPP <- model_nhood*model_random*(strm_r_n)*slope*(rd_1_co) #+ rd_acc*(model_nhood*model_random)
...
diff_rd <- rd_2-rd_1
#after rasterising data just read file using the below codes
writeRaster(diff_rd, file = sprintf("xxx/%s/rasterised/10_B_4_diff_rd.rst",city_name),overwrite = T)
writeRaster(diff_sim, file = sprintf("xxx/%s/rasterised/10_B_4_diff_sim.rst",city_name),overwrite = T)
writeRaster(rd_1, file = sprintf("xxx/%s/rasterised/10_B_4_rd_1.rst",city_name),overwrite = T)
writeRaster(rd_2, file = sprintf("xxx/%s/rasterised/10_B_4_rd_2.rst",city_name),overwrite = T)
writeRaster(sim99, file = sprintf("xxx/%s/rasterised/10_B_4_sim99.rst",city_name),overwrite = T)
...
f1 <- 2*((pre*rec)/(pre+rec))
f1
D_R_S <- jaccard(diff_rd, diff_sim, x.min = 0.7, y.min = 0.7)
N_D_R_S <- jaccard(diff_sim, nhood_2, x.min = 0.7, y.min = 0.7)
...
result_tab <- data.frame(name = city_name, 
                         ...
                         diff_b = b1,
                         diff_d = d1,
                         jacc_full_real_sim = R_S,
                         jacc_diff_real_sim = D_R_S,
                         jacc_diff_neighofreal_sim = N_D_R_S)
write.table(result_tab, file = 'xxx/table/result_tab_10', 
            append = T, sep = '\t', row.names = F, col.names = F)
...
for (i in 1:total){#begin running simulation
  ...
  model_TPP <- model_nhood*model_random*(rd_1_co)
  ...
  writeRaster(T1, file = sprintf("xxx/%s.tif",city_name,(start_year+i)),overwrite = T)
  dev.off()
...
#after rasterising data just read file using the below codes
writeRaster(diff_rd, file = sprintf("xxx/%s/rasterised/10_B_5_diff_rd.rst",city_name),overwrite = T)
writeRaster(diff_sim, file = sprintf("xxx/%s/rasterised/10_B_5_diff_sim.rst",city_name),overwrite = T)
writeRaster(rd_1, file = sprintf("xxx/%s/rasterised/10_B_5_rd_1.rst",city_name),overwrite = T)
writeRaster(rd_2, file = sprintf("xxx/%s/rasterised/10_B_5_rd_2.rst",city_name),overwrite = T)
writeRaster(sim99, file = sprintf("xxx/%s/rasterised/10_B_5_sim99.rst",city_name),overwrite = T)
